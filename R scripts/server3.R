server <- function(input, output) {
  observeEvent(input$goButton, {
    ...
    #### Filtering based on dropdowns - Time Race 1 ####
    ...
    output$plot_ProgSpeed_AvStkRate <-  renderPlotly({
      ...
      fig <- plot_ly()
      fig <- fig %>% add_lines(x = tab_AvVel()[,"variable"], y = tab_AvVel()[,"ProgSpeed"], name = "Prognostic %", line = list(color='rgb(22, 96, 167)'))
      fig <- fig %>% add_lines(x = tab_AvStkRate()[,"variable"], y = tab_AvStkRate()[,"value"], name = "Stroke Rate (spm)", yaxis = "y2", line = list(color = 'rgb(205, 12, 24)'))
    ...
    #### output ProgSpeed and Av Stk Rate A2 ####
    output$plot_ProgSpeed_AvStkRate_A2 <-  renderPlotly({
      ay <- list(
        overlaying = "y",
        side = "right",
        title = "Stroke Rate (s/min)",
      ...
      fig <- plot_ly()
      fig <- fig %>% add_lines(x = tab_AvVel()[,"variable"], y = tab_AvVel()[,"ProgSpeed"], name = "Race 1 Prognostic %", line = list(color='rgb(22, 96, 167)'))
      fig <- fig %>% add_lines(x = tab_AvStkRate()[,"variable"], y = tab_AvStkRate()[,"value"], name = "Race 1 Stroke Rate (spm)", yaxis = "y2", line = list(color = 'rgb(205, 12, 24)'))
      
      fig <- fig %>% add_lines(x = tab_AvVel_A2()[,"variable"], y = tab_AvVel_A2()[,"ProgSpeed"], name = "Race 2 Prognostic %", line = list(color='rgb(22, 96, 167)', dash = "dot"))
      fig <- fig %>% add_lines(x = tab_AvStkRate_A2()[,"variable"], y = tab_AvStkRate_A2()[,"value"], name = "Race 2 Stroke Rate (spm)", yaxis = "y2", line = list(color = 'rgb(205, 12, 24)', dash = "dot"))
    ...
    output$plot_scatter_SR_Progspeed <-  renderPlot({
      plotdata <-  data.frame(tab_AvStkRate()[["value"]], tab_AvVel()[["ProgSpeed"]])
      ...
      lineA = geom_smooth(aes(colour = input$Name), method=lm, se=FALSE, fullrange=TRUE)
      pointsB = geom_point(data = plotdata2, aes(x=StrokeRate, y=ProgSpeed), colour="blue")
      lineB = geom_smooth(data = plotdata2, aes(x=StrokeRate, y=ProgSpeed, colour = 'Top10'), method=lm, se=FALSE, fullrange=TRUE)
    ...
    #### output Scatter SR vs Progspeed Athlete 2 ####
    output$plot_scatter_SR_Progspeed_A2 <-  renderPlot({
      plotdata <-  data.frame(tab_AvStkRate()[["value"]], tab_AvVel()[["ProgSpeed"]])
      names(plotdata)[1]<-paste("StrokeRate")
      names(plotdata)[2]<-paste("ProgSpeed")
      plotdata2 <-  data.frame(tab_AvStkRate_A2()[["value"]], tab_AvVel_A2()[["ProgSpeed"]])
      names(plotdata2)[1]<-paste("StrokeRate")
      names(plotdata2)[2]<-paste("ProgSpeed")
    ...
    #Race 1
      Filtered_AccTime_data <-  tab_AccTime() %>% select(variable, value)
    ...
    AvgVel = tab_AvVel() %>% summarize(Avg_Velocity = mean(value, na.rm = TRUE)) %>% round(2)
    ClassProgSpeed2 = round(ClassProgSpeed,2)
    VelDifference = round(AvgVel/ClassProgSpeed*100,2)

    #Race 2
    Filtered_AccTime_data_A2 <-  tab_AccTime_A2() %>% select(variable, value)
    Filtered_AccTime_data_A2 <- column_to_rownames(Filtered_AccTime_data_A2,'variable') 
    FinishTime_A2 <- Filtered_AccTime_data_A2["2000",]
    PrognosticTime_A2 <- 2000/ClassProgSpeed
    Difference_A2 <-  round(PrognosticTime_A2/FinishTime_A2*100,2)
    FinishTime_A2 <- sec_to_ms(FinishTime_A2)
    PrognosticTime_A2 <- sec_to_ms(PrognosticTime_A2)
    AvgVel_A2 = tab_AvVel_A2() %>% summarize(Avg_Velocity_A2 = mean(value, na.rm = TRUE)) %>% round(2)
    ClassProgSpeed2_A2 = round(ClassProgSpeed,2)
    VelDifference_A2 = round(AvgVel_A2/ClassProgSpeed*100,2)
    ...
    Race_2 = data.frame(FinishTime_A2, PrognosticTime_A2, Difference_A2, AvgVel_A2, ClassProgSpeed2_A2, VelDifference_A2)
    ...
    ({datatable(RaceSummary, rownames = NULL, options = list(dom='t', ordering=F, scrollX = TRUE))})
    })
    ...
    Athlete1 <- reactive({
      ...
      Filtered_AvProgSpeed_data <-  tab_AvVel() %>% select(variable, ProgSpeed)
      ...
      Athlete1 <-  data.table("Distance (m)" = c("250", "500", "750", "1000", "1250", "1500", "1750", "2000", "", "Average", "Max", "Min", "% Drop Off"), 
    ...
    Athlete2 <- reactive({
      ...
      Filtered_AccTime_data <-  tab_AccTime_A2() %>% select(variable, value)
      Filtered_AccTime_data <- column_to_rownames(Filtered_AccTime_data,'variable')
      ...
      ### Splits Stroke calculations ####
      Filtered_AvStkRate_data <-  tab_AvStkRate_A2() %>% select(variable, value)
      Filtered_AvStkRate_data <- column_to_rownames(Filtered_AvStkRate_data,'variable')
      #250 splits
      ...
      Filtered_AvProgSpeed_data <-  tab_AvVel_A2() %>% select(variable, ProgSpeed)
      Filtered_AvProgSpeed_data <- column_to_rownames(Filtered_AvProgSpeed_data,'variable')
      ...
      Athlete2 <-  data.table("Distance (m)" = c("250", "500", "750", "1000", "1250", "1500", "1750", "2000", "", "Average", "Max", "Min", "% Drop Off"), 
                              "250m splits (secs)" = c(Split250_Time_250, Split250_Time_500, Split250_Time_750, Split250_Time_1000, Split250_Time_1250, Split250_Time_1500, Split250_Time_1750, Split250_Time_2000,"", Split250_Time_Avg, Split250_Time_Max, Split250_Time_Min, Split250_Time_DO),
                              "500m splits (secs)" = c("", Split500_Time_500, "", Split500_Time_1000, "", Split500_Time_1500, "", Split500_Time_2000, "", Split500_Time_Avg, Split500_Time_Max, Split500_Time_Min, Split500_Time_DO),
                              "Avg Velocity (m/s)" = c(Split250_AvVel_250, Split250_AvVel_500, Split250_AvVel_750, Split250_AvVel_1000, Split250_AvVel_1250, Split250_AvVel_1500, Split250_AvVel_1750, Split250_AvVel_2000, "", Split250_AvVel_Avg, Split250_AvVel_Max, Split250_AvVel_Min,Split250_AvVel_DO),
                              "Avg Stroke Rate (s/min)" = c(Split250_AvStkRate_250, Split250_AvStkRate_500, Split250_AvStkRate_750, Split250_AvStkRate_1000, Split250_AvStkRate_1250, Split250_AvStkRate_1500, Split250_AvStkRate_1750, Split250_AvStkRate_2000, "", Split250_AvStkRate_Avg, Split250_AvStkRate_Max, Split250_AvStkRate_Min, Split250_AvStkRate_DO),
    ...
    Top10 <- reactive({
      ...
      Filtered_AccTime_data <-  tab2_AccTime() %>% select(variable, Average)
      Filtered_AccTime_data <- column_to_rownames(Filtered_AccTime_data,'variable')
      ...
      ### Splits Stroke calculations ####
      Filtered_AvStkRate_data <-  tab2_AvStkRate() %>% select(variable, value)
      Filtered_AvStkRate_data <- column_to_rownames(Filtered_AvStkRate_data,'variable')
      #250 splits
      ...
      Filtered_AvProgSpeed_data <-  tab2_AvVel() %>% select(variable, ProgSpeed)
      Filtered_AvProgSpeed_data <- column_to_rownames(Filtered_AvProgSpeed_data,'variable')
    ...
    if (uiOutput("select_Class") == "Single Athlete"){
      
      
      
      
    }else if(uiOutput("select_Class") == "Two Athletes"){
      
      
      
      
      
    }else if (uiOutput("select_Class") == "vs Top 10"){
    ...
    ### SummaryTableTop10  ####
     output$SummaryTable <-  DT::renderDataTable({ 
       Summarydatatable <-  data.table(Athlete1()[,"Distance (m)"], 
                                       Athlete1()[,"250m splits (secs)"], "Ideal" = Top10()[,"250m splits (secs)"],
                                       Athlete1()[,"500m splits (secs)"], "Ideal" = Top10()[,"500m splits (secs)"],
                                       Athlete1()[,"Avg Velocity (m/s)"], "Ideal" = Top10()[,"Avg Velocity (m/s)"],
                                       Athlete1()[,"Avg Stroke Rate (s/min)"], "Ideal" = Top10()[,"Avg Stroke Rate (s/min)"],
                                       Athlete1()[,"Avg Prog Speed (%)"], "Ideal" = Top10()[,"Avg Prog Speed (%)"])
       ...
       ({datatable(Summarydatatable2,rownames = NULL, options = list(dom='t', ordering=F, scrollX = TRUE, pageLength = 13)) %>% formatStyle(
    ...
    output$SummaryTable2 <-  DT::renderDataTable({ 
      Summarydatatable2 <-  data.table(Athlete1()[,"Distance (m)"], 
                                      "Race 1" = Athlete1()[,"250m splits (secs)"], "Race 2" = Athlete2()[,"250m splits (secs)"],
                                      "Race 1" = Athlete1()[,"500m splits (secs)"], "Race 2" = Athlete2()[,"500m splits (secs)"],
                                      "Race 1" = Athlete1()[,"Avg Velocity (m/s)"], "Race 2" = Athlete2()[,"Avg Velocity (m/s)"],
                                      "Race 1" = Athlete1()[,"Avg Stroke Rate (s/min)"], "Race 2" = Athlete2()[,"Avg Stroke Rate (s/min)"],
                                      "Race 1" = Athlete1()[,"Avg Prog Speed (%)"], "Race 2" = Athlete2()[,"Avg Prog Speed (%)"])
      
      Summarydatatable2 <-  setnames(Summarydatatable2, old = c("Race 1.250m splits (secs)","Race 2.250m splits (secs)", "Race 1.500m splits (secs)","Race 2.500m splits (secs)", "Race 1.Avg Velocity (m/s)","Race 2.Avg Velocity (m/s)", "Race 1.Avg Stroke Rate (s/min)","Race 2.Avg Stroke Rate (s/min)", "Race 1.Avg Prog Speed (%)","Race 2.Avg Prog Speed (%)"), new = c("250m splits (secs)","Race 2", "500m splits (secs)","Race 2", "Avg Velocity (m/s)","Race 2", "Avg Stroke Rate (s/min)","Race 2", "Avg Prog Speed (%)", "Race 2"))
      ...
      #### Output headers ####
    output$Summarydatatable2head <- renderText({paste(input$Class,input$Name, input$Competition, input$Date, input$Phase, "vs", input$Name2, input$Competition2, input$Date2, input$Phase2)})
